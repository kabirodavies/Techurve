        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {categories?.map((category, index) => {
            ...
            return (
              <div
                key={category?._id}
                className="relative bg-white p-5 flex items-center gap-4 rounded-lg shadow-sm group transition-all duration-300 hover:shadow-lg hover:scale-[1.025] border-t border-b border-transparent hover:border-t-shop_orange/40 hover:border-b-shop_orange/40"
              >
                <div className="relative border-t border-b border-shop_orange/30 hover:border-t-shop_orange hover:border-b-shop_orange w-20 h-20 bg-white flex-shrink-0 flex items-center justify-center">
                  <Link href={`/category/${category?.slug?.current}`} className="block w-full h-full flex items-center justify-center">
                    <Icon className="w-14 h-14 text-shop_dark_blue group-hover:scale-110 transition-transform duration-300" aria-label={iconKey || "default"} />
                  </Link>
                </div>
